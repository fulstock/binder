...
def Ace2json(entity_dir, rel_dir, token_dict_offset1, token_dict_offset2, txtfn, docs, nercount, relcount, sentcount):
    ...
    for relation in rel_dir:
        ...
        # if rel_dir[relation]["relation"]== "DISC": continue
        if token_dict_offset1[entity_dir[arg1]['offset'][0]]['sentid'] != token_dict_offset1[entity_dir[arg2]['offset'][0]]['sentid']:
            pdb.set_trace()
        ...
        # if rel_dir[relation]["relation"]== "DISC":
            # print token_dict_offset1[entity_dir[arg1]['offset'][0]]['sentid'], token_dict_offset1[entity_dir[arg2]['offset'][0]]['sentid'],token_dict_offset2[entity_dir[arg1]['offset'][1]]['sentid'], token_dict_offset2[entity_dir[arg2]['offset'][1]]['sentid'] 
        relcount += 1
...
for i in range(5):
    ...
    relcount = 0
    sentcount = 0
    for stanfordfn in glob.glob('./corpus/test'+str(i)+'/' + '*.stanford.so'):    
    ...
    outfn = '../data/ace04/test/'+str(i) + '.json'
...
for i in range(5):
    ...
    relcount = 0
    sentcount = 0
    for stanfordfn in glob.glob('./corpus/train'+str(i)+'/' + '*.stanford.so'):    
        txtfn = stanfordfn.replace('stanford.so','txt')
        annfn = stanfordfn.replace('stanford.so','ann')
        entity_dir, rel_dir = parseAce(annfn, entity_set, rel_set)
        parses, token_dict_offset1, token_dict_offset2 = parseStanford(stanfordfn)
        nercount, relcount, sentcount = Ace2json(entity_dir, rel_dir, token_dict_offset1, token_dict_offset2, txtfn, docs, nercount, relcount, sentcount)
    outfn = '../data/ace04/train/'+str(i) + '.json'
    WriteDocs(docs, outfn)
