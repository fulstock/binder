...
class ModelArguments:
    ...
    checkpoint_path: str = field(
        metadata={"help": "Path to saved model, for inference."}
    )
...
def main():
    ...
    # model = Binder(config)
    model = Binder.from_pretrained(model_args.checkpoint_path, config = config)
    ...
    def prepare_train_features(examples):
        ...
        if data_args.predict_entities_id_file and data_args.predict_logits_input_file:
            examples_with_ids = []
            with open(data_args.predict_entities_id_file, "r", encoding = "UTF-8") as eif:
                for line in eif:
                    example_with_id = json.loads(examples_with_ids)
                    examples_with_ids[example_with_id["id"]] = {**example_with_id}
            with open(data_args.predict_logits_input_file, "r") as lf:
                mask_dict = json.load(lf)
            for eid, mask in mask_dict.items():
                examples_with_ids[eid]["mask"] = mask
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            for entity_type, start_char, end_char in zip(entity_types, entity_start_chars, entity_end_chars):
                ...
                if offsets[text_start_index][0] <= start_char and offsets[text_end_index][1] >= end_char:
                    ...
                    elif data_args.predict_entities_id_file and data_args.predict_logits_input_file:
                        pass
