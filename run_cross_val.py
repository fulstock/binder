...
class DataTrainingArguments:
    ...
    predict_neutrals_input_file: str = field(
        default = "", metadata = {"help": "Predicted neutrals for masking."}
    )
    neutrals_entity_data_file: str = field(
        default = "", metadata = {"help": "Predicted neutrals data for masking."}
    )
...
def main():
    ...
    if data_args.do_cross_validation:
        ...
        folds = KFold(n_splits=data_args.cross_validation_folds, shuffle = True)
        splits = folds.split(train_dataset)

        metrics = []

        for s_idx, (train_idxs, val_idxs) in enumerate(splits):
            ...
            def prepare_train_features(examples):
                ...
                ### examples: 
                # text : [str] -- list of texts
                # entity_types [[str]] -- List of lists of entity types
                # entity_start_chars: [[int]] -- list of lists of start positions
                # entity_end_chars: [[int]] -//-
                # id [str] -- list of ids
                # word_start_chars [[int]]
                # word_end_chars [[int]]
                ...
                # print("Tokenized examples keys:", tokenized_examples.keys())

                ### Tokenized examples keys: 
                # 'input_ids'
                # 'token_type_ids' 
                # 'attention_mask'
                # 'offset_mapping'
                ...
                if len(data_args.predict_neutrals_input_file) > 0:

                    neutrals_data = {}

                    import json

                    with open(data_args.predict_neutrals_input_file, "r", encoding = "UTF-8") as nf:
                        for line in nf:
                            neutral_line = json.loads(line)
                            line_id = neutral_line["id"]
                            text_id, e_id = line_id.split('_fe')
                            if text_id in neutrals_data:
                                neutrals_data[text_id][e_id] = {
                                    "entity_text" : neutral_line["text"],
                                    "neutral_ners" : neutral_line["pred_ner"]
                                }
                            else:
                                neutrals_data[text_id] = {
                                    e_id : {
                                        "entity_text" : neutral_line["text"],
                                        "neutral_ners" : neutral_line["pred_ner"]
                                    }
                                }
                    with open(data_args.neutrals_entity_data_file, "r", encoding = "UTF-8") as df:
                        for line in df:
                            neutral_line = json.loads(line)
                            line_id = neutral_line["id"]
                            text_id, e_id = line_id.split('_fe')
                            assert neutrals_data[text_id][e_id]["entity_text"] == neutral_line["text"]
                            neutrals_data[text_id][e_id] = {
                                **neutral_line,
                                **neutrals_data[text_id][e_id]
                            }
                ...
                for i, offsets in enumerate(offset_mapping):
                    ...
                    text_id = examples["id"][sample_index]
                    ...
                    for entity_type, start_char, end_char in zip(entity_types, entity_start_chars, entity_end_chars):
                        ...
                        if offsets[text_start_index][0] <= start_char and offsets[text_end_index][1] >= end_char:
                            ...
                            elif len(data_args.predict_neutrals_input_file) > 0:
                                this_text_entities = neutrals_data[text_id]

                                this_entity = [e for e in this_text_entities.values() if e["original_entity_start_chars"][0] == start_char and e["original_entity_end_chars"][0] == end_char]
                                if len(this_entity) != 1:
                                    print("What")
                                    print(this_entity)
                                    print(this_text_entities)
                                    print(start_char, end_char, text[start_char : end_char], text, text_id)
                                    raise ValueError

                                this_entity = this_entity[0]
                                filtered_neutrals = [(s, e, t) for s, e, t, _ in this_entity["neutral_ners"] if t == entity_type]
                                entity_type_id = entity_type_str_to_id[entity_type]

                                for ns, ne, nt in filtered_neutrals:

                                    start_token_index, end_token_index = text_start_index, text_end_index
                                    # Move the start_token_index and end_token_index to the two ends of the span.
                                    # Note: we could go after the last offset if the span is the last word (edge case).
                                    while start_token_index <= text_end_index and offsets[start_token_index][0] <= ns:
                                        start_token_index += 1
                                    start_token_index -= 1

                                    while offsets[end_token_index][1] >= ne:
                                        end_token_index -= 1
                                    end_token_index += 1
            ...
            assert not any([t in val_idxs or v in train_idxs for t in train_idxs for v in val_idxs])

            cross_train_examples = train_dataset.select(train_idxs)
            cross_eval_examples = train_dataset.select(val_idxs)
            cross_test_examples = cross_eval_examples

            # Create train feature from dataset
            with training_args.main_process_first(desc="train dataset map pre-processing"):
                cross_train_dataset = cross_train_examples.map(
            ...
            with training_args.main_process_first(desc="validation dataset map pre-processing"):
                cross_eval_dataset = cross_eval_examples.map(
            ...
            with training_args.main_process_first(desc="prediction dataset map pre-processing"):
                cross_test_dataset = cross_test_examples.map(
            ...
            trainer = BinderTrainer(
                ...
                eval_dataset=cross_eval_dataset,
                eval_examples=cross_eval_examples,
                tokenizer=tokenizer,
            ...
            max_train_samples = (
                data_args.max_train_samples if data_args.max_train_samples is not None else len(cross_train_dataset)
            )
            train_metrics["train_samples"] = min(max_train_samples, len(cross_train_dataset))
            ...
            max_eval_samples = data_args.max_eval_samples if data_args.max_eval_samples is not None else len(cross_eval_dataset)
            eval_metrics["eval_samples"] = min(max_eval_samples, len(cross_eval_dataset))

            trainer.log_metrics("eval", eval_metrics)
            trainer.save_metrics("eval", eval_metrics)

            logger.info("*** Predict ***")
            results = trainer.predict(cross_test_dataset, cross_test_examples)
            test_metrics = results.metrics

            max_predict_samples = (
                data_args.max_predict_samples if data_args.max_predict_samples is not None else len(cross_test_dataset)
            )
            test_metrics["predict_samples"] = min(max_predict_samples, len(cross_test_dataset))
    ...
    cross_validation_metrics = {k : np.mean([metrics[f_idx]["eval"][k] for f_idx in range(data_args.cross_validation_folds)]) for k in metrics[0]["eval"].keys()}

    logger.info("*** Cross-Validation Result ***")
    logger.info(str(cross_validation_metrics))
    print(json.dumps(cross_validation_metrics, ensure_ascii = False, indent = 2))
    with open("./cross-val-res.json", "w", encoding = "UTF-8") as cf:
        json.dump(cross_validation_metrics, cf, ensure_ascii = False, indent = 2)
